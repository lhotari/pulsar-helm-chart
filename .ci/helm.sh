...
function ci::wait_function_running() {
    ...
    while [[ ${num_running} -lt 1 ]]; do
      ...
      ${KUBECTL} get events --sort-by=.lastTimestamp -A | tail -n 30 || true
      podname=$(${KUBECTL} get pods -l component=function -A --no-headers -o custom-columns=":metadata.name") || true
      if [[ -n "$podname" ]]; then
...
function ci::wait_message_processed() {
    ...
    podname=$(${KUBECTL} get pods -l component=function -A --no-headers -o custom-columns=":metadata.name")
    counter=1
